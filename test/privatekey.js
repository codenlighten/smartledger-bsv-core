...
describe('PrivateKey', function () {
  ...
  it('should create a private key from WIF buffer', function () {
    const a = new PrivateKey(Base58Check.decode('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m'))
    should.exist(a)
    should.exist(a.bn)
  })
  ...
  describe('instantiation', function () {
    ...
    it('should not be able to instantiate private key WIF is too long', function () {
      expect(function () {
        const buf = Base58Check.decode('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m')
        const buf2 = Buffer.concat([buf, Buffer.alloc(0x01)])
    ...
    it('should not be able to instantiate private key WIF because of unknown network byte', function () {
      expect(function () {
        const buf = Base58Check.decode('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m')
        const buf2 = Buffer.concat([Buffer.from('ff', 'hex'), buf.slice(1, 33)])
        return new PrivateKey(buf2)
  ...
  describe('#json/object', function () {
    ...
    it('should input/output json', function () {
      ...
      const key = PrivateKey.fromJSON(JSON.parse(json))
      // toObject() is the deliberate export and still round-trips exactly...
      JSON.stringify(key.toObject()).should.equal(json)
      // ...but JSON.stringify() must NOT emit the secret. This test used to assert that
      // it did, which is how a private key reached an OP_RETURN via the GDAF anchor path:
    ...
    it('rejects a malformed bn instead of silently deriving another key', function () {
      ...
      const cases = ['zzzz', '[REDACTED]', 'nothex', '12g4']
  ...
  describe('argument handling that used to produce the wrong key', function () {
    ...
    it('honours the network passed to fromString instead of discarding it', function () {
      ...
      const k = PrivateKey.fromString(HEX, 'testnet')
    ...
    it('agrees with the constructor for WIF input', function () {
      const wif = new PrivateKey(HEX, 'testnet').toWIF()
    ...
    it('fromHex and the constructor return the SAME key', function () {
      ...
      const b = PrivateKey.fromHex(HEX)
      const c = PrivateKey.fromBuffer(Buffer.from(HEX, 'hex'))
  ...
  describe('does not leak the secret through JSON', function () {
    it('redacts bn from JSON.stringify', function () {
      ...
      const scalar = k.bn.toString('hex')
    ...
    it('redacts it when the key is nested inside another object', function () {
      const k = new PrivateKey()
      const payload = JSON.stringify({ context: { signer: k } })
