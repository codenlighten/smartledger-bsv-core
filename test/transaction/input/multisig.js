...
const MultiSigInput = bsv.Transaction.Input.MultiSig
...
describe('MultiSigInput', function () {
  ...
  it('can estimate how heavy is the output going to be', function () {
    const transaction = new Transaction()
      ...
      .to(address, 1000000)
    const input = transaction.inputs[0]
    input._estimateSize().should.equal(188)
  })
  ...
  it('can parse list of signature buffers, from TX signed with key 1 and 2', function () {
    ...
    transaction.inputs[0].isValidSignature(transaction, transaction.inputs[0].signatures[0]).should.equal(true)
    transaction.inputs[0].isValidSignature(transaction, transaction.inputs[0].signatures[1]).should.equal(true)
  })
  it('can parse list of signature buffers, from TX signed with key 3 and 1', function () {
    ...
    transaction.inputs[0].isValidSignature(transaction, transaction.inputs[0].signatures[0]).should.equal(true)
    transaction.inputs[0].isValidSignature(transaction, transaction.inputs[0].signatures[2]).should.equal(true)
  })
})
