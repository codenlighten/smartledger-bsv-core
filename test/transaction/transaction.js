...
describe('Transaction', function () {
  ...
  it('should serialize and deserialize correctly a given transaction', function () {
    const transaction = new Transaction(tx1hex)
  ...
  describe('adding inputs', function () {
    ...
    describe('_clearSignatures with custom-script inputs', function () {
      ...
      it('still invokes clearSignatures on typed subclass inputs', function () {
        const tx = new Transaction()
          ...
          .change(changeAddress)
          .sign(privateKey)
        const clearSpy = sinon.spy(tx.inputs[0], 'clearSignatures')
        tx._clearSignatures()
    ...
    describe('P2PKH-prefixed outputs (1Sat Ordinals, MAP+BAP, etc.)', function () {
      ...
      it('Script.isPublicKeyHashOutPrefix matches canonical P2PKH', function () {
        const canonical = bsv.Script.buildPublicKeyHashOut(bsv.Address.fromString(toAddress))
      ...
      it('Script.isPublicKeyHashOutPrefix matches P2PKH + trailing data', function () {
        const ord = buildOrdinalScript(toAddress)
      ...
      it('Transaction.from dispatches a 1Sat ordinal output to PublicKeyHashInput', function () {
        const addr = new bsv.PrivateKey(privateKey).toAddress().toString()
        const ordUtxo = {
          txid: 'a'.repeat(64),
          outputIndex: 0,
          satoshis: 100000,
        ...
        const tx = new Transaction().from(ordUtxo)
      ...
      it('sighash is computed against the FULL locking script (envelope included)', function () {
        ...
        const verified = tx.inputs[0].isValidSignature(tx, {
  ...
  describe('change address', function () {
    ...
    it('fee per kb can be set up manually', function () {
      const inputs = _.map(_.range(10), function (i) {
