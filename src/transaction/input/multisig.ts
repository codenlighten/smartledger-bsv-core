...
import type { MultiSigInput as MSInput, MultiSigInputConstructor } from '../types'
 ...
 * @constructor
 */
const MultiSigInput = function MultiSigInput (this: MSInput, input: Record<string, unknown>, pubkeys?: unknown[], threshold?: number, signatures?: unknown[]) {
...
MultiSigInput.prototype.toObject = function (this: MSInput): Record<string, unknown> {
  ...
  _.each(this.publicKeys, function (publicKey: PublicKey) {
    if (publicKey.toString() === privateKey.publicKey.toString()) {
      results.push(new TransactionSignature({
        ...
        inputIndex: index,
        signature: Sighash.sign(transaction, privateKey, sigtype, index, (self.output as OutputType).script, (self.output as OutputType).satoshisBN),
        sigtype
      }))
    }
  })
...
MultiSigInput.prototype._updateScript = function (this: MSInput): MSInput {
  this.setScript(scriptClass().buildMultisigIn(
    this.publicKeys,
    this.threshold,
...
MultiSigInput.prototype.clearSignatures = function (this: MSInput): void {
  this.signatures = new Array(this.publicKeys.length)
  this._updateScript()
}

MultiSigInput.prototype.isFullySigned = function (this: MSInput): boolean {
  return this.countSignatures() === this.threshold
}

MultiSigInput.prototype.countMissingSignatures = function (this: MSInput): number {
  return this.threshold - this.countSignatures()
}

MultiSigInput.prototype.countSignatures = function (this: MSInput): number {
...
MultiSigInput.prototype.isValidSignature = function (this: MSInput, transaction: Transaction, signature: TransactionSignature): boolean {
  ...
  return Sighash.verify(
    ...
    (this.output as OutputType).script,
...
MultiSigInput.prototype._estimateSize = function (this: MSInput): number {
  const scriptSize = 1 + this.threshold * (MultiSigInput.SIGNATURE_SIZE as number)
